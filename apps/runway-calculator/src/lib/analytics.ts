...
const TOOL_SLUG = 'runway-calculator';
 ...
 * Track social shares
 */
export function trackSocialShare(platform: string, source: string, metadata?: Record<string, string | number>): void {
  trackEvent('export_click', {
    action: 'social_share',
    category: 'engagement',
  ...
  // Send to backend
  sendToBackend('social_share', platform, { source, ...metadata });
  // Send to IdeaLift funnel
  sendToIdeaLift('tool_use', { action: 'social_share', platform, ...metadata });
}